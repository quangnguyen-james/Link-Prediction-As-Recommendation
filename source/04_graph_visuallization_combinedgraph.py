...
G_combined = nx.read_graphml(r"graph\combined_graph.graphml")

# Tạo bảng màu ngẫu nhiên cho từng ego
ego_colors = {}
for node, data in G_combined.nodes(data=True):
    egos = data["ego_group"]  # Lấy danh sách ego của nút
    if egos:
...
# Gán màu cho các nút
node_colors = [
    ego_colors[data["ego_group"][0]] if data["ego_group"] else "#CCCCCC"  # Màu mặc định cho nút không thuộc ego
...
plt.title("Facebook Combined Graph - Colored by Ego Clusters", fontsize=16)
plt.show()
